...
/* 
*	delta_dist_x :	this serves as the distance the ray has to travel to go from 1 x-side to the next x-side
*	delta_dist_y :	xthis serves as the distance the ray has to travel to go from 1 y-side to the next y-side
...
*		3.2 if we hit a wall, we need to set the hit to true and store the wall number/type
*	4. Calculate the distance to the wall
*		4.1 We need to calculate the distance to the wall based on the side we hit
*		4.2 With this dist we can calculate the height of the wall strip
*			This is the way to simulate the depth
*		4.3 We need to calculate the texture x coordinate that we will use to get 
